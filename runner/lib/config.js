...
const cfg = {
  ...
  repoPath: process.env.MRAPI_REPO_PATH || path.join(os.homedir(), 'Documents', 'GitHub', 'mrapi-dev-orchestrator'),
  brainChatUrlW01: process.env.MRAPI_W01_CHAT_URL || '',
  chromeUserDataDir: process.env.MRAPI_CHROME_PROFILE_DIR || path.join(os.homedir(), 'AppData', 'Local', 'MRAPI', 'chrome-w01'),
  chromeChannel: process.env.MRAPI_CHROME_CHANNEL || 'chrome',
  brainTimeoutMs: Math.max(60000, Number(process.env.MRAPI_BRAIN_TIMEOUT_SECONDS || 600) * 1000),
  codexCommand: process.env.MRAPI_CODEX_COMMAND || '',
  codexTimeoutMs: Math.max(60000, Number(process.env.MRAPI_CODEX_TIMEOUT_SECONDS || 1800) * 1000)
};
